...
class Form
{
    ...
    public static function form(string $action, $id = null, $attributes = [], array $items = [], array $submit = []) :string
    {
        ...
        foreach($items as $name => $item) {
            ...
            if (isset($item['options']['riot'])) {
                ...
                $innerHtml = HtmlUtil::tag($riotTag, [], '') . HtmlUtil::tag('script', [], "riot.mount('${riotTag}', ${riotOptJson})");
        ...
        $html .= HtmlUtil::tag($tag, $_attributes, $title);
        $html .= '</form>';
        return $html;
    }
    ...
    public static function formStart(string $action, $attributes = [], array $validationRules = []) :string
    {
        ...
        $html = HtmlUtil::tagStart('form', $_attributes);
    ...
    public static function formInput(string $label, string $name, array $attributes = [], array $options = []) :string
    {
        ...
        if ($_options['inputOnly'] ?? false) {
            return HtmlUtil::tag('input', $_attributes);
        } else {
            return Form::_formWrap($label, HtmlUtil::tag('input', $_attributes), $_options);
        }
    }
    ...
    public static function formTextarea(string $label, string $name, array $attributes = [], array $options = []) :string
    {
        ...
        if ($_options['inputOnly']) {
            return HtmlUtil::tag('textarea', $_attributes, $_value);
        } else {
            return Form::_formWrap($label, HtmlUtil::tag('textarea', $_attributes, htmlspecialchars($_value)), $_options);
        }
    }
    ...
    public static function formSelect(string $label, string $name, array $list, array $attributes = [], array $options = []) :string
    {
        ...
        $selectHtml = HtmlUtil::tagStart('select', $_attributes);
        ...
        $selectHtml .= HtmlUtil::tagEnd('select');
        if ($_options['inputOnly']) {
            return $selectHtml;
        } else {
    ...
    public static function formAlert($errors, array $options = []) :string
    {
        ...
        if (is_array($errors) && count($errors) > 0) {
            $output = HtmlUtil::tag('div', ['class' => $_options['class']], nl2br(implode("\n", $errors)));
        }
        return $output;
    }
    ...
    private static function _formWrap($label, $inputHtml, $_options) :string
    {
        ...
        $output = HtmlUtil::tagStart('div', ['class' => $class]);
        $output .= HtmlUtil::tag('label', ['class' => 'control-label'], $label);
        $output .= $inputHtml;
        if ($_options['help'] ?? false) {
            $output .= HtmlUtil::tag('p', ['class' => 'help-block'], $_options['help']);
        }
        $output .= HtmlUtil::tagEnd('div');
        return $output;
    }
